#[cfg(all(not(any(target_os = "solana", target_os = "zkvm")), feature = "curve25519"))]
...
impl Address {
    #[cfg(any(any(target_os = "solana", target_os = "zkvm"), feature = "std"))]
    ...
    /// # Ok::<(), anyhow::Error>(())
    /// ```
    // If any(target_os = "solana", target_os = "zkvm"), then the function will use
